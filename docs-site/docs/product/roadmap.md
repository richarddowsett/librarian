...
This roadmap outlines the three development milestones based on a security-first framework, using **React Native / Expo** to target Web, iOS, and Android from a single codebase, and leveraging **AWS Cognito & DynamoDB** for security, fine-grained access control, and cloud syncing.

```mermaid
graph TD
    M1["Milestone 1: Security & Foundation<br>(Cognito Auth, DynamoDB Isolation, WAF Setup)"] --> M2["Milestone 2: Functional Core & UI<br>(Barcode Scanning, Catalog CRUD, Search)"]
...
Before implementing any functional business logic, a secure foundation must be established. This phase focuses on setting up the workspace, configuring identity and access management, deploying WAF protections, and securing data storage via IAM fine-grained access policies.
...
3. **Database Security & Fine-Grained Access Control**:
   - Define AWS DynamoDB single/multi-table schemas for users, books, and series.
   - Deploy IAM Fine-Grained Access Control policies enforcing absolute user data isolation (`dynamodb:LeadingKeys` restricted to `${cognito-identity.amazonaws.com:sub}`). Users can only read, write, or delete their own library records.
